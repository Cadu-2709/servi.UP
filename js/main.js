import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
        ...
        async function loadUserSettings() {
            ...
            userSettings = docSnap.exists() ? docSnap.data() : { companyName: 'Sua Empresa', cnpj: '', address: '', phone: 'Seu Telefone', logoUrl: 'https://placehold.co/200x100/cccccc/ffffff?text=Logo' };
            applySettings();
        }
        ...
        function showView(viewId, id = null, clientId = null) {
            ...
            if (viewId === 'view-tools') {
                    showToolsMenu(); // Garante que o menu seja sempre a primeira visão
                }
        }
        
        function renderClientsList(clientsToRender) { // <-- Aceita um parâmetro
        ...
        clientsToRender.forEach(client => { // <-- Usa o parâmetro
    ...
    // Pega o valor atual do campo de busca e do filtro de status ativo
    ...
    let filtered = budgets.filter(b => b.status !== 'paid'); // Começa com a base de orçamentos não pagos
    ...
    // 2. Em seguida, aplica a BUSCA de texto na lista já filtrada por status
    if (searchTerm) {
    ...
    // O resto da função continua exatamente igual, renderizando a lista 'filtered'
    ...
    <div class="text-right flex-grow">
        ...
        // No seu js/main.js, substitua a função renderReports por esta:

function renderReports() {
    const detailsContainer = document.getElementById('details-list-container');
    const summaryContainer = document.querySelector('#view-reports .grid'); // Container dos totais
    const customPeriodFields = document.getElementById('custom-period-fields');

    // Identifica qual filtro está ativo
    currentPeriodFilter = document.querySelector('#report-filters .bg-\\[var\\(--cor-primaria\\)\\]').dataset.period;

    // Esconde/mostra os campos de data e o resumo de totais
    customPeriodFields.classList.toggle('hidden', currentPeriodFilter !== 'custom');
    summaryContainer.classList.toggle('hidden', currentPeriodFilter === 'by_client');

    if (currentPeriodFilter === 'by_client') {
        // --- LÓGICA DO NOVO RELATÓRIO POR CLIENTE ---
        detailsContainer.innerHTML = '';
        
        const faturamentoPorCliente = {};

        // 1. Calcula o faturamento para cada cliente
        budgets.filter(b => b.status === 'paid').forEach(budget => {
            if (!faturamentoPorCliente[budget.clientId]) {
                const clientInfo = clients.find(c => c.id === budget.clientId);
                faturamentoPorCliente[budget.clientId] = {
                    name: clientInfo?.name || budget.clientName || "Cliente Removido",
                    total: 0
                };
            }
            faturamentoPorCliente[budget.clientId].total += budget.paidAmount || 0;
        });

        // 2. Converte o objeto em um array e ordena do maior para o menor faturamento
        const rankingClientes = Object.values(faturamentoPorCliente).sort((a, b) => b.total - a.total);

        if (rankingClientes.length === 0) {
            detailsContainer.innerHTML = '<p class="text-center text-gray-500 mt-4">Nenhum faturamento registrado para clientes.</p>';
            return;
        }

        // 3. Exibe a lista na tela
        rankingClientes.forEach((cliente, index) => {
            const rankingClass = index === 0 ? 'bg-yellow-100' : (index === 1 ? 'bg-gray-200' : (index === 2 ? 'bg-yellow-50' : 'bg-gray-50'));
            const itemHTML = `
                <div class="flex justify-between items-center ${rankingClass} p-3 rounded-md">
                    <div class="flex items-center">
                        <span class="font-bold text-lg w-8">${index + 1}º</span>
                        <p class="font-semibold">${cliente.name}</p>
                    </div>
                    <p class="font-bold text-green-700 text-lg">${formatCurrency(cliente.total)}</p>
                </div>
            `;
            detailsContainer.innerHTML += itemHTML;
        });
        
    } else {
        // --- LÓGICA ANTIGA DO RELATÓRIO POR PERÍODO ---
        ...
        function renderExpensesList() {
            ...
            if (expenses.length === 0) {
                container.innerHTML = `<p class="text-center text-gray-500 mt-8">Nenhuma despesa registada.</p>`;
                return;
            }
            expenses.sort((a, b) => b.date - a.date).forEach(expense => {
                const clone = template.content.cloneNode(true);
        ...
        document.getElementById('app-container').addEventListener('click', (e) => {
            ...
            if (!target) return;
            // Dentro de document.getElementById('app-container').addEventListener('click', (e) => { ... });
...
const toolBtn = e.target.closest('.tool-menu-btn');
    ...
    if (e.target.closest('#tools-back-btn')) {
        ...
        if (isToolVisible) {
            showToolsMenu(); // Se uma ferramenta está aberta, volte para o menu de ferramentas
        } else {
            showView('view-dashboard'); // Se o menu está aberto, volte para o dashboard
        }
    ...
    // Filtra a lista principal de clientes que temos na memória
    const filteredClients = clients.filter(client => 
        client.name.toLowerCase().includes(searchTerm)
    );
    
    // Renderiza a lista novamente, mas agora apenas com os clientes filtrados
...
// Adicione este listener no final do seu arquivo js/main.js
...
document.getElementById('calc-btn').addEventListener('click', () => {
    ...
    resultadoContainer.innerHTML = `
        ...
        <p class="mt-1 text-gray-700">Valor com <span class="font-bold text-green-600">acréscimo</span>: <span class="font-bold">${formatCurrency(comAcrecimo)}</span></p>
    `;
});
...
document.getElementById('image-upload').addEventListener('change', (e) => {
    ...
    // Mostra o container de preview e o loader
    previewContainer.classList.remove('hidden');
    canvasLoader.classList.remove('hidden');
    
    // Desabilita o botão de download enquanto processa
    ...
    // Permite que a logo (de outro domínio) seja carregada no canvas
    logoImage.crossOrigin = "Anonymous";

    const reader = new FileReader();

    // Quando o arquivo do usuário for lido
    reader.onload = (event) => {
        userImage.src = event.target.result;
    };

    // Quando a imagem do usuário terminar de carregar
    userImage.onload = () => {
        // Agora que a imagem principal carregou, definimos o SRC da logo
        logoImage.src = userSettings.logoUrl; 
    };
    
    // Quando a logo também terminar de carregar, podemos desenhar
    logoImage.onload = () => {
        // Define o tamanho do canvas igual ao da foto do usuário
        canvas.width = userImage.width;
        canvas.height = userImage.height;

        // 1. Desenha a foto do usuário no canvas
        ctx.drawImage(userImage, 0, 0);

        // 2. Calcula o tamanho e a posição da logo
        const padding = canvas.width * 0.05; // 5% de margem
        const logoHeight = canvas.height * 0.15; // Logo terá 15% da altura da imagem
        const logoWidth = (logoImage.width / logoImage.height) * logoHeight;
        const x = canvas.width - logoWidth - padding;
        const y = canvas.height - logoHeight - padding;
        
        // Coloca uma opacidade na logo para efeito de marca d'água
        ctx.globalAlpha = 0.85; 

        // 3. Desenha a logo por cima da foto
        ctx.drawImage(logoImage, x, y, logoWidth, logoHeight);

        // Reseta a opacidade
        ctx.globalAlpha = 1.0;

        // 4. Prepara o botão de download
        downloadBtn.href = canvas.toDataURL('image/png'); // Gera o link da imagem final
    ...
    // Inicia o processo lendo o arquivo selecionado
    reader.readAsDataURL(file);
});

// Adicione este bloco inteiro no final do seu arquivo js/main.js

// --- LÓGICA DO GERADOR DE PIX ---

// Função auxiliar para formatar os campos do BR Code
const formatField = (id, value) => {
    const len = value.length.toString().padStart(2, '0');
    return `${id}${len}${value}`;
};

// Função principal que monta o texto do BR Code
const generateBRCode = (pixKey, pixName, pixCity, amount, txid) => {
    ...
    const currencyCode = formatField('53', '986'); // 986 = BRL
    
    const transactionAmount = formatField('54', amount.toFixed(2));
    const countryCode = formatField('58', 'BR');
    
    const merchantName = formatField('59', pixName.substring(0, 25)); // Limite de 25 caracteres
    const merchantCity = formatField('60', pixCity.substring(0, 15)); // Limite de 15 caracteres
    ...
    const crc16 = '6304'; // CRC16 estático para simplificar, funciona na maioria dos casos.
...
document.getElementById('pix-generate-btn').addEventListener('click', () => {
    ...
    const txid = document.getElementById('pix-id').value.replace(/\s/g, '') || '***'; // Remove espaços ou usa '***'
    ...
    qr.make();
    const qrCodeDataUrl = qr.createDataURL(6, 4); // (tamanho do módulo, margem)
...
function showToolsMenu() {
    ...
    // Esconde todas as ferramentas individuais
...
function showTool(toolName) {
    ...
    // Mostra apenas a ferramenta clicada
